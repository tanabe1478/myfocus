...
- **3ペインUI**: [Reeder](https://reederapp.com/) を参考にした落ち着いたデザイン。設定からライト/ダークテーマを切り替え可能。組み込みテーマを複製して色を編集し、JSONで共有できるユーザー定義テーマ対応（[設計詳細](docs/themes.md)）
- **全文検索（⌘K）**: SQLite FTS5によるタイトル・要約・フィード名の検索。検索クエリを保存し、サイドバーからいつでも再実行できる
- **AIアシスタント**: [Pi coding agent](https://pi.dev/) をRPCモードで組み込み。読んでいる記事についての相談、ローカル記事アーカイブの横断検索、スター・既読傾向と「興味あり／なし」に基づく未読記事の推薦、読んでいる記事を起点にしたローカル関連記事の発見、新しい記事・フィードの探索ができる。「今日のブリーフィング」はサイドバーから開け、推薦は12時間キャッシュされる（新着取得時は自動無効化）。提案された記事やフィードはワンクリックで開ける。記事要約はバックグラウンドキューで生成し、左の[AI要約受信箱](docs/summary-inbox.md)からまとめて確認できる
- **ローカル完結**: 記事はSQLiteに永続化。起動中は15分ごとに全フィードを自動更新し、フィードの窓から記事が流れ落ちる前にローカルへ確保する
- **OPMLインポート**: Inoreader等からのエクスポートファイルを取り込み可能（並列フェッチ）
- **フィード分類**: 追加時のカテゴリ指定、既存フィードの移動、カテゴリ名変更、未分類化、ドラッグ＆ドロップに対応（[詳細](docs/feed-categories.md)）
- **フィード自動発見**: サイトのURLを入れると `<link rel="alternate">` からフィードURLを検出
- **ドッグフーディング診断ログ**: 設定からローカルJSONLログを有効化し、保存先の表示・フォルダー表示・削除が可能。本文やAIプロンプトは記録しない（[詳細](docs/diagnostic-logging.md)）
